...
Pour donner une image concrète d'un système aux degrés de liberté en interactions nous introduisons le système le plus simple à appréhender que l'on appelle le modèle d'Ising à deux dimensions et qui permet de décrire le magnétisme des matériaux aimantés. 
On se place dans l'espace à trois dimensions repéré par la base cartésienne $\{\ev_\nu\}_{\nu=x,y,z}$ de $\R^3$. On considère un réseau carré infini dans le plan $(\ev_x, \ev_y)$. Sur chaque noeud du réseau se trouve un spin à une composante qui se représente par un vecteur dont la direction est parallèle à $\ev_z$ et dont l'orientation peut être selon $+\ev_z$ ou $-\ev_z$. On note $\mathscr{R} = \left\{ m \ev_x + n \ev_y \, | \, (m, n) \in \N^2 \right\}$ l'ensemble des positions des noeuds et donc des spins\footnote{Le pas du réseau est pris valant $1$, ou autrement dit, toutes les longueurs sont exprimées en unité du pas du réseau}. Pour représenter le sens du spin à la position $\rv \in \mathscr{R}$, on lui associe le scalaire $S_\rv$ qui vaut $+1$ s'il est selon $+\ev_z$ et $-1$ s'il est selon $-\ev_z$, ce sont les degrés de liberté du système\footnote{Le réseau est de dimensions infinies, de ce fait il s'y trouve une infinité de degré de liberté. Un système macroscopique possédant un nombre gigantesque ($\sim 10^{24}$) de degrés de liberté, cela est une modélisation cohérente de la réalité.}. On appelle \textit{micro état} ou \textit{configuration} du système un ensemble $\Sr = \{ S_\rv \}_{\rv \in \mathscr{R}}$ donné : une représentation en terme de vecteurs en est faite en \refig{schemaIsing}. Pour un micro état $\Sr$ on peut alors définir son énergie $H\(\Sr\)$ associée selon
\begin{equation}
H\(\Sr = \{ S_\rv \}_{\rv \in \mathscr{R}} \) = -J \sum_{\left<\rv, \rv'\right>}S_\rv S_{\rv'} \, .
\label{eq:HamIsing}
\end{equation}
La notation $\left<\rv, \rv'\right>$ signifie que l'on somme sur tout $\rv \in \mathscr{R}$ et $\rv' \in \mathscr{R}$  mais que le terme $S_\rv S_{\rv'}$ contribue à la somme si et seulement les neouds $\rv$ et $\rv'$ sont plus proches voisins (i.e si $\rv' = \rv \pm \ev_x$ ou $\rv' = \rv \pm \ev_y$). Le paramètre $J$ représente une énergie, c'est une constante positive.
...
Si le système est placé dans un milieu à température nulle et initialement dans un micro-état quelconque il évolue alors vers le micro-état minimisant son énergie, donc minimisant $H$. Comme $J$ est positif ceci se produit pour $\Sr_1 = \{1,1,1 .., 1\}$ ou $\Sr_{-1} = \{-1,-1,-1 .., -1\}$, micro-états dans lesquels l'ensemble des spins sont tous orientés dans la même direction (notons qu'en fait $\Sr_1 = \Sr_{-1}$ puisque le système est symétrique par l'échange de $\ev_z$ en $-\ev_z$). Ceci permet aussi aussi de montrer ce qu'un système aux degrés de liberté en interactions représente. En effet considérons le système dans le micro-état $\Sr_1$ et on imagine que l'on puisse tourner un spin, i.e. imposer $S_\rv=-1$ pour un $\rv$ donné. Alors, pour minimiser l'énergie, ses quatre spins plus proches voisins vont le "contraindre" à revenir dans la position $S_\rv = 1$ : ils sont en interaction. 

Lorsque l'on soumet maintenant le système à une température $T$ non nulle, il reçoit de l'énergie thermique de l'extérieur qui tend à favoriser la désorganisation : son état d'équilibre n'est plus un micro-état particulier. En revanche on peut définir la probabilité qu'a l'équilibre le système soit dans le micro-état $\Sr$ par
\begin{equation}
	p\(\Sr\) = \frac{1}{\Zc}\exp\(-\frac{H\(\Sr\)}{k_B T} \)
\end{equation}
avec la notation
\begin{equation}
\Zc = \sum_{\Sr'}  \exp\(-\frac{H\(\Sr'\)}{k_B T} \) \, . 
\end{equation}
Le scalaire $\Zc$ est appelé la fonction de partition du système, il s'agit d'une somme sur l'ensemble des micro-états possibles. La constante $k_B$ est la constante de Boltzmann. Plus la température est élevée plus l'énergie thermique reçue que l'on peut évaluer par $\sim k_bT$ est grande par rapport à l'énergie $J$. Pour $k_BT \ll J$ seuls $\Sr_1$ et des micro-état proches de $\Sr_1$ ont une probabilité non nulle d'être visité par le système. En revanche, lorsque $k_BT \gg J$ les spins reçoivent suffisamment d'énergie thermique pour que le système puisse être indifféremment dans n'importe quel micro-état à l'équilibre. 
...
En particulier une des grandeurs caractéristique du système est son aimantation moyenne définie par
\begin{equation}
	m  \equiv \left< S_\rv \right>	\equiv \sum_{\Sr} p\(\Sr\) S_\rv \, .
\end{equation}
Le modèle d'Ising représente le ...
Nous nous intéressons à cette grandeur car elle va nous permettre d'introduire le concept de transition de phase. En effet, dans le cas où $T=0$ nous savons que $p(\Sr_1) = 1$ et donc $m =1$. En revanche pour $T$ non nul le désordre apporté par l'énergie thermique fait que dans l'ensemble $m \to 0$ quand $T \to 0$.  
Il se trouve que l'on peut tracer expérimentalement l'évolution de $m$ en fonction de la température extérieure $T$. On obtient alors la courbe bleu de \refig{aimantation} ayant une cassure au niveau d'une température que l'on note $T_c$ (pour température critique) et que l'on définira plus précisément un peu plus loin. L'aimantation ne se comporte donc pas à la manière de la courbe rouge comme il aurait été possible de le penser : on n'a pas seulement $m \to 0$ quand $T \to 0$ mais $m=0$ pour $T>T_c$. Cette évolution marque la présence d'une transition de phase. Pour bien comprendre ce dont il s'agit définissons ensuite ce qu'est de manière générale une transition de phase et comment les étudier. 

\begin{figure}[H]
\begin{center}
\includegraphics[width=0.95\columnwidth]{aimantation.pdf}
\caption{Evolution de l'aimantation $m$ du système en fonction la température. (bleu continu) courbe obtenue expérimentalement. (rouge discontinu) ce qui aurait été attendu.}
...
\subsubsection{Transitions du second ordre}
...
Dans le cas du modèle d'Ising cette fonction représente l'influence que deux spins $S_\rv$ et $S_{\rv'}$ ont l'un sur l'autre et elle est plus exactement définie par
\begin{equation}
	G^{(2)}(|\rv - \rv'|) \equiv \left< S_\rv S_{\rv'} \right>  \equiv \sum_{\Sr}  p\(\Sr\) S_\rv S_{\rv'}  \, ,
\end{equation}
...
\subsection{Approche de mécanique statistique}
