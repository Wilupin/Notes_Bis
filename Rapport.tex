...
Brouillon ... Nous étudions les phénomènes de transitions de phases et plus particulièrement les transitions du second ordre produites par un changement de température. Lorsqu'une telle transition se produit dans un milieu pour une température dite critique $T_c$, ces propriétés physico-chimiques changent brusquement. Il est alors possible de définir des grandeurs physiques que l'on appelle $\eta$ et $\nu$  qui ont la particularité d'être identiques pour deux milieux différents partageant des propriétés microscopiques de symétries communes. Le formalisme du groupe de renormalisation non perturbatif associé à l'approximation dite BMW doit permettre de calculer numériquement $\eta$, $\nu$ et $T_c$ pour différents systèmes à travers la résolution d'équation integro-différentielles non linéaire. Notre but a été d'essayer de corriger un premier code de simulation de ces équations calculant $\eta$ et $\nu$ pour une première classe de système et d'en écrire un second pour calculer $T_c$ d'un système particulier ...
\end{abstract}
...
En thermodynamique on appelle phase un milieu possédant des propriétés physiques et chimiques homogènes. Or, avec la modification de certains paramètres comme la température, la pression extérieure ou le champ magnétique extérieur, (dans notre étude on ne considère que les effets de la température), un système peut changer de phase. Il se produit alors une transition de phase. On appelle $T_c$, température critique, la température à laquelle se produit cette transition. Ce changement de phase est dû à une réorganisation, au niveau microscopique, des constituants  du système (molécules, atomes, spins ...), ayant un effet macroscopique. Landau a classé ces transitions suivant deux types : celles du premier ordre et celles du second \cite{toledano1987landau}. La distinction se fonde sur la continuité d'une fonction de la température que l'on appelle le paramètre d'ordre, au moment de la transition. \\

Dans l'exemple du modèle d'Ising la température permet de réorganiser la structure microscopique en modifiant la "quantité de désordre" des spins. A champ magnétique extérieur nul et à faible $T$, i.e. $T< T_c$, ils ont tendance à rester alignés et ordonnés, le système est dans une phase pour laquelle $m \neq 0$ dite ferromagnétique. En revanche pour $T$ grand, i.e. $T>T_c$ les spins ont un comportement désordonné, il s'agit d'une seconde phase où $m = 0$ appelée paramagnétique. De plus l'aimantation correspond ici au paramètre d'ordre du système. C'est une fonction continue avec la température : il s'agit d'une transition du second ordre. Une transition du premier ordre aurait eu un paramètre d'ordre évoluant comme la courbe de \refig{aimantation2}, le passage de l'eau de l'état liquide à l'état gazeux par augmentation de la température à pression extérieure constante est un exemple d'une telle transition. 
 \begin{figure}[H]
\begin{center}
...
\begin{equation}
	G^{(2)}(|\rv_1 - \rv_2|) = \frac{1}{\Zc(T, \Bc)} \frac{\partial^2 \Zc(T, \Bc)}{\partial b_\rv \partial b_{\rv'}} \, .
\end{equation}
...
Il s'avère cependant que la définition même des fonctions de partition est trop complexe pour permettre de faire des calculs directement avec. Il y a alors deux possibilités, soit il est possible de simplifier leur expression, soit il est nécessaire de faire des approximations et/ou des calculs numériques approchés. Le modèle d'Ising 2D en est un exemple de cas où la fonction de partition peut être simplifiée, ceci a été fait par Onsager \cite{Onsager}. Cependant il s'agit d'un cas exceptionnel, pour la très grande majorité des systèmes cela n'est pas possible et il est nécessaire d'avoir recours à des approximations. \\

Nous avons donc utilisé une méthode d'approximation numérique des fonctions de partition utilisant les outils du groupe de renormalisation. Nous détaillerons ces méthodes en \refsec{RG}. Cela nécessite cependant d'étudier des systèmes qui ne sont pas décrit par des micro-états, ensembles de valeurs discrètes (i.e. $\{S_\rv\}_\rv$ comme nous l'avons vu pour le modèle d'Ising 2D), mais par des champs. Nous introduisons donc les concepts fondamentaux de mécanique statistique des champs. \\
...
Nous introduisons dans cette sections le formalisme de la physique statistique \cite{diu2007thermodynamique} des champs sans en entrer dans les détails. Ceci est nécessaire pour pouvoir aborder de manière générale l'étude des transitions de phase et des exposants critiques à l'aide de l'outil du groupe de renormalisation (RG). \\
...
\end{equation}
et il est dit $\mathbb{Z}_2$ si $H[-\varphi] = H[\varphi]$. Le modèle d'Ising 2D est un exemple aussi de modèle $\mathbb{Z}_2$ puisque rappelons que dans sons cas
\begin{equation}
	H( \{-S_\rv\}_\rv) = H( \{S_\rv\}_\rv) 
\end{equation}
...
Dans un second temps, en \refsec{Ising}, \refsec{NumIsing} et\refsec{ResIsng}, en nous avons repris le modèle d'Ising 2D et nous avons essayé de déterminer sa température critique $T_c$ à l'aide de l'approximation BMW toujours. Nous savons que ce modèle ne s'exprime, a priori, pas avec des champs. Nous avons donc du le ré-exprimer différemment à l'aide d'une méthode bien connue pour permettre de l'étudier tout de même en théorie des champs avec le NPRG, nous montrons ceci en \refsec{Ising}. On rappelle que la fonction de partition de ce système peut être simplifiée (grâce aux travaux d'Onsager \cite{Onsager} et donc toutes les grandeurs liées à la transition de phase peuvent être calculées de manière exacte, en particulier la température critique $T_c$. Nous avons alors pu comparer la $T_c$ théorique à celle donnée par la résolution numérique que nous avons entièrement écrite et détaillée \refsec{NumIsing}. Le but est ici de montrer que le NPRG est bien capable de calculer des grandeurs non universelles comme $T_c$, ce qui est théoriquement possible comme nous l'avons vu, mais très difficile dans la pratique pour des modèles réalistes.\\


Avant de rentrer dans les détails de ces deux études distinctes nous introduisons les concepts de RG et NPRG dans la prochaine section \refsec{RGGeneral} afin de bien comprendre les résolutions détaillées par la suite et utilisant ces méthodes.
...
Nous introduisons de manière synthétique les principales idées du RG développé par Wilson \cite{wilson1971renormalization, wilson1971renormalization2,fisher1998renormalization}. Commençons pour cela par définir la transformée de Fourier du champ $\varphiv$ appartenant à $L^2(\Omega)$ par 
\begin{equation}
\begin{split}
...
Ces résultats sont ceux qui avaient déjà été trouvé avec le même code avant que nous le ré-organisions. Ce sont des valeurs en très bon accord avec les valeurs théoriques attendues : $\eta = 0.25$ et $\nu = 1.01$.  \\
...
On retrouve ainsi la formulation d'un problème de théorie des champs que l'on peut résoudre avec le NPRG et l'approximation BMW. Concernant la mise en pratique, la seule différence avec tout ce qui a été étudié précédemment c'est qu'ici, par TFSD, les intégrales sur les moments ne sont plus calculés sur un domaine infini mais sur "la première zone de Brillouin", i.e. sur la surface $[-\pi, \pi]^2$. Comme le modèle d'Ising 2D possède une symétrie\footnote{en effet $H$ est invariant par l'échange de $\ev_z$ en $-\ev_z$ comme nous avons pu le voir dans l'introduction. \\
...
La fonction $U$ représente ce que l'on appelle le potentiel du système dans l'approximation de champ moyen. Et ainsi le potentiel $V_k(\phi) = \Gamma_k(\pv, \phi)$ que l'on peut définir dans les équations BMW prend en $k=\Lambda$ la valeur $V_\Lambda = U$. La fonction $\eo$ s'appelle quant à elle la relation de dispersion. Dans le modèle continu $\varphiv^4$ il y avait de même une relation de dispersion qui valait alors $\varepsilon(\pv) = \pv^2$.\\
