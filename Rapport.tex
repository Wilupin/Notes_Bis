...
Enfin, la recherche de la température critique se fait en variant le paramètre directement lié à la température $\tilde{\beta}$ qui intervient dans les conditions initiales des équations BMW. On cherche donc $\tilde{\beta}_c = 2d/(k_BT_c)$ pour lequel les équations BMW \textit{adimensionnées} possèdent une solution particulière dite de \textit{point fixe} (c.f. \refsec{ptfixe}). Si, pour $\tilde{\beta}$ introduit dans la condition initiale des équations BMW \textit{adimensionnées}, on obtient la solution de point fixe comme solution alors nous savons que $\tilde{\beta}=\tilde{\beta}_c$ (et que $T = 2d/(k_B\tilde{\beta})$ vaut $T_c$).\\
...
\begin{align}
	...
	\end{aligned}
\end{align}
où l'on note $X_k(\phi) = \partial_{\phi}^2 V_k(\phi)$. Les notations $J_n$ et $I_n$ correspondent à celle du modèle $\varphiv^4$ (c.f. \refann{Ising2D}). On rappelle que $\partial_t = k\partial_k$.\\
\indent
