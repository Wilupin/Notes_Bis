...
De manière générale l'hamiltonien $H$ ne s'exprime pas seulement en fonction du champ $\varphiv$ mais son expression contient aussi des constantes de couplage qui caractérisent la physique du système. Dans le modèle d'Ising 2D, $J$ est une constante de couplage par exemple, dans la théorie $\varphiv^4$ ce sont $r_0$ et $u_0$. Ces constantes sont notées ici $\{g_i\}_i$ avec $i \in \N$. On écrit alors $H[\varphiv] = H[\varphiv ; \{g_i\}]$. L'hamiltonien $H_k[\varphiv_{k,<}]$ peut s'exprimer aussi sous la forme $H_k[\varphiv_{k,<} ; \{g_{k,i}\}_i]$ où $\{g_{k,i}\}_i$ sont de nouvelles constantes de couplage. \\
 
On introduit à l'itération $p \in \N$ - itération à laquelle on a intégré sur $[k_p, \Lambda]$ - l'opérateur $O_p$ qui envoie $\{g_i\}_i$ sur $\{g_{k_p,i}\}_i$ (les constantes de couplages obtenu à l'itération $p$). L'hypothèse fondamentale du RG, qui n'est pas prouvée mais toujours vérifiée, est qu'il existe un point fixe\footnote{Ce point fixe existe en fait si l'on adimensionne les $g_{k_p,i}$, c'est à dire qu'à chaque itération on fait en sorte que ce soit des variables sans dimension de longueur. On note parfois $\tilde{g}_{k_p,i}$ les constantes adimensionnées.} de $O_p$ pour $p \to \infty$, si l'on est à la température critique. Ceci signifie plus exactement qu'il existe $\{g^*_i\}_i$ tel que 
 \begin{equation}
 	\lim\limits_{k \rightarrow \infty}  g_{k,i} = \lim\limits_{\substack{p \to \infty \\ d\Lambda \to 0 }}  g_{k_p,i} =  g^*_i
 \end{equation} 
 
Sans entrer plus en profondeur dans les calculs du RG on admet que l'on peut montrer que l'existence de ce point fixe explique l'universalité des exposants critiques \cite{Delamotte2012}. Deux systèmes avec les mêmes propriétés de symétries auront le même point fixe, et donc une physique identique à la transition de phase. \\
 
Dans un calcul de RG on prend un nombre fini de constantes de couplage et on considère qu'elles sont suffisamment faibles pour faire des développements du hamiltonien en puissances de ces constantes à chaque itération. Ce sont ces approximations qui permettent de faire des calculs mais elles limitent les applications possibles. Le NPRG développe alors une technique reprenant la même idée de calcul mais permettant plus de précision.
...
Pour que cela fonctionne le régulateur doit satisfaire de nombreuses contraintes. Dans le modèle $O(N)$ il suffit de prendre $\Rc_{k,ij} = \Rc_k$ comme une simple fonction et pas un tenseur. En considérant que $\Rc_k(\qv) = \Rc_k(q)$ avec $q = \|\qv\|_2$ la norme 2 dans $\R^d$ (ce qui correspond à ce que nous utiliserons), les régulateurs respectant toutes ces contraintes ont l'allure de la courbe de  \refig{RegDerReg} (a).
\begin{figure}[H]
\begin{center}
...
\end{equation}
En utilisant $\Zc_k$ et $W_k$ nous pouvons définir aussi des fonctions de corrélations comme nous l'avons fait pour le modèle d'Ising 2D et qui était à la base de la définition des coefficients $\eta$ et $\nu$. Il est possible de se reporter à \refann{} pour plus de détails, nous considérons simplement ici qu'il est possible de définir un opérateur $G_{k}$ que l'on nomme le propagateur et qui est égal à un opérateur de corrélations à deux points, telle qu'au sens de l'inverse d'opérateur (c.f \refann{maths})
\begin{equation}
