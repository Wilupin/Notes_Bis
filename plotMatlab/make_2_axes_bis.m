function [h_fig, h_a, h_b] = make_2_axes_bis()
...
left_space_2 = 0.08;
v_space = 0.215;
fig_b_start = left_space_1+norm_ax_width+left_space_2-0.05;
v_space_bis = v_space + 0.05;
...
h_a = axes('Units','normalized',...
    ...
    'FontName', f_name);
dim = [0.2 0.2 0.3 0.3];
h_ann_a = annotation('textbox',dim,'String','(a)','FitBoxToText','on');
set(h_ann_a, 'Units', 'normalized');
set(h_ann_a, 'EdgeColor', 'none');
set(h_ann_a, 'FontSize', t_fsz);
set(h_ann_a, 'Position', [left_space_1+norm_ax_width-0.07 v_space_bis 0 norm_ax_height]);
...
h_b = axes('Units','normalized',...
     ...
     'FontSize', fsz,...
     'FontName', f_name);
 h_ann_b = annotation('textbox',dim,'String','(b)','FitBoxToText','on');
